...
export const Clickable = ({ as = 'div', disabled, tooltip, tooltipSide, onClick, children, ...props }) => {
  const child = h.h(Interactive, _.merge({
    as, disabled,
    onClick: (...args) => onClick && !disabled && onClick(...args)
  }, props), [children])

  if (tooltip) {
    return h.h(TooltipTrigger, { content: tooltip, side: tooltipSide }, [child])
  } else {
    return child
  }
}
...
export const tabBar = ({ activeTab, tabNames, refresh = _.noop, getHref }, children = []) => {
  ...
  return h.div({ style: Style.tabBar.container }, [
...
export const LabeledCheckbox = ({ checked, onChange, disabled, children, ...props }) => {
  return h.h(Fragment, [
    h.h(Checkbox, { checked, onChange, disabled, ...props }),
    h.h(Interactive, {
      as: 'h.span',
...
export const RadioButton = ({ text, labelStyle, ...props }) => {
  ...
  return h.h(Fragment, [
    ...
    h.label({ htmlFor: id, style: labelStyle }, text)
  ])
}

export const spinnerDefault = ({ outerStyles = {}, innerStyles = {} }) => h.div(
  {
    style: {
      position: 'absolute',
      display: 'flex', alignItems: 'center',
...
export const comingSoon = h.span({
  style: {
 ...
 * @param {Array} props.options - can be of any type; if objects, they should each contain a value and h.label, unless defining getOptionLabel
 */
